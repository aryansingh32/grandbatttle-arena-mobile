...
class AuthStateManager extends ChangeNotifier {
  ...
  void initAuthListener() {
    ...
    _authSubscription = FirebaseAuth.instance.authStateChanges().listen(
      (user) {
        _user = user;
        print(
          '>>> authStateChanges: user=${user?.uid ?? "null"} email=${user?.email}',
        );
  ...
  Future<bool> signInWithEmail({
    ...
    print('📧 AuthStateManager: signInWithEmail() called for: $email');
    _clearError();
    _setLoadingAndNotify(true);

    try {
      print(
        '📧 AuthStateManager: Calling FirebaseAuthService.signInWithEmail()...',
      );
      ...
      print(
        '📧 AuthStateManager: Firebase result - success=${result.isSuccess}, error=${result.error}',
      );

      if (result.isSuccess) {
        print('✅ AuthStateManager: Email sign-in successful!');
        print('✅ User: ${result.user?.uid}, email: ${result.user?.email}');
        print('✅ Waiting for authStateChanges() stream to emit...');

        // ✅ CRITICAL: Force a reload to ensure stream emits
        if (result.user != null) {
          try {
            await result.user!.reload();
            print('✅ AuthStateManager: User reloaded successfully');
          } catch (e) {
            print('⚠️ AuthStateManager: User reload failed (non-fatal): $e');
          }
        }

        // Don't clear loading yet - auth stream will handle it
        // The _authSubscription listener will receive the update and call notifyListeners()
        return true;
      } else {
        print('❌ AuthStateManager: Email sign-in failed: ${result.error}');
      ...
      print('❌ AuthStateManager: Email sign-in exception: $e');
      _setLoadingAndNotify(false);
  ...
  Future<bool> signUp({
    ...
    try {
      print(
        '📝 AuthStateManager: Starting sign-up for: $email at ${DateTime.now()}',
      );
      ...
      print('📝 AuthStateManager: signUp() returned at ${DateTime.now()}');

      if (result.isSuccess) {
        print('✅ AuthStateManager: Sign-up successful!');
        print('✅ User: ${result.user?.uid}, email: ${result.user?.email}');

        // ✅✅✅ CRITICAL FIX: Force reload to trigger stream
        if (result.user != null) {
          try {
            await result.user!.reload();
            print('✅ AuthStateManager: User reloaded after sign-up');

            // ✅ Update local state immediately (belt-and-suspenders approach)
            _user = FirebaseAuth.instance.currentUser;
            print('✅ AuthStateManager: Local _user updated to ${_user?.uid}');
          } catch (e) {
            print('⚠️ AuthStateManager: User reload failed (non-fatal): $e');
          }
        }
        ...
        print('✅ AuthStateManager: Waiting for auth stream to propagate...');
        return true;
      } else {
        print('❌ AuthStateManager: Sign-up failed: ${result.error}');
        _setLoadingAndNotify(false);
        _error = result.error;
        notifyListeners();
        return false;
      ...
      print('❌ AuthStateManager: Sign-up exception: $e');
      _setLoadingAndNotify(false);
  ...
  Future<bool> signInWithGoogle({
    ...
    _setLoadingAndNotify(
      true,
    ); // ensure this helper exists or replace with _isLoading = true; notifyListeners();
    _error = null;
    notifyListeners();
    ...
    try {
      ...
      print(
        '🔵 signInWithGoogle: got googleUser: ${googleUser.email}, id=${googleUser.id}',
      );
      ...
      print(
        '🔥 FirebaseAuthException in signInWithGoogle: ${fae.code} ${fae.message}',
      );
      return false;
    } catch (e, st) {
